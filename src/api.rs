            ...
            for item in arr.iter_mut() {
                if let Some(obj) = item.as_object_mut() {
                    for hub in &hubs {
                        ...
                        let qty = obj.get("product_qty").and_then(|v| v.as_i64()).unwrap_or(1) as f64;
                        let cost = obj.get("mat_costs").and_then(|p| p.get(hub)).and_then(|v| v.as_f64()).unwrap_or(0.0);
                        obj.insert(format!("profit_{}", &hub_lower), serde_json::Value::from(revenue * qty - cost));
                    }
                }
            }
        }
...
async fn enrich_blueprints(
    ...
    if bps_raw.is_empty() { return Vec::new(); }
    ...
    for bp in &bps_raw {
        ...
        // Product info from manufacturing data
        ...
        if product_id != 0 {
            price_jita = hub_prices.get("Jita").and_then(|h| h.get(&product_id)).copied().unwrap_or(0.0);
    ...
    results.sort_by(|a, b| {
        a.get("bp_name").and_then(|v| v.as_str()).unwrap_or("")
            .cmp(b.get("bp_name").and_then(|v| v.as_str()).unwrap_or(""))
    });
    results
}
